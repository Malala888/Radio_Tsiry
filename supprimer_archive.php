...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['Supprimer'])) {
        ...
        if ($nouveauNom && $type && $date_debut && $date_fin && $situation && $type_payement && $montant !== false && $matin && $midi && $soir && $nbr_diffusion !== false) {
            ...
            $sql = 'UPDATE `archives` SET `nom` = :nouveauNom, `type` = :type, `date_debut` = :date_debut, `date_fin` = :date_fin, `situation` = :situation, `type_payement` = :type_payement, `montant` = :montant, `matin` = :matin, `midi` = :midi, `soir` = :soir, `nbr_diffusion` = :nbr_diffusion WHERE `nom` = :ancienNom';
            $query = $db->prepare($sql);
        ...
        $sql = 'DELETE FROM `archives` WHERE `nom` = :nom';
        $query = $db->prepare($sql);
            ...
            <section class="col-12">
                ...
                <form method="post" style='margin-left: 225px;'>
                    ...
                    <div>
                        ...
                        <a href="archives.php" class="btn btn-primary ml-3">Retour</a> 
                    </div>
                </form>
            </section>
        </div>
