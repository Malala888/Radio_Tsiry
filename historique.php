...
<body>
    ...
    try {
        ...
        // Récupérer les données des 12 derniers mois à partir de la table 'medias' et 'archives'
        $query = "SELECT MONTH(DatePaye) as mois, 
                    SUM(montant) as total_recette 
              FROM (
                    SELECT DatePaye, montant FROM medias 
                    UNION ALL 
                    SELECT DatePaye, montant FROM archives
              ) as combined 
              WHERE DatePaye >= DATE_SUB(CURDATE(), INTERVAL 12 MONTH)
              GROUP BY mois";
